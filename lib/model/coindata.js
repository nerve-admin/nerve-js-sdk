...
var CoinData = function CoinData(bufferReader) {
  this.fromList = [];
  this.toList = [];
  ...
  for (var _i = 0; _i < toCount; _i++) {
    this.toList.push(new CoinTo(bufferReader));
  }
};

var CoinFrom = function CoinFrom(bufferReader) {
...
var CoinTo = function CoinTo(bufferReader) {
  this.address = bufferReader.readBytesByLength();
  this.assetsChainId = bufferReader.readUInt16LE();
  this.assetsId = bufferReader.readUInt16LE();
  this.amount = bufferReader.readBigInteger();
...
CoinData.prototype.getPrintInfo = function () {
  ...
  for (var _i2 = 0; _i2 < this.toList.length; _i2++) {
    if (_i2 > 0) {
      result += ",";
    }

    result += this.toList[_i2].getPrintInfo();
  }
